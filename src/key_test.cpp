...
void test_key(const T& x, const T& y) {
   auto keyx = sysio::convert_to_key(x);
   auto keyy = sysio::convert_to_key(y);
...
void test_compare() {
   ...
   using namespace sysio::literals;
   test_key("a"_n, "a"_n);
   test_key(name(), name());
   test_key("a"_n, "b"_n);
   test_key("ab"_n, "a"_n);
   ...
   test_key(public_key(std::in_place_index<0>, sysio::ecc_public_key{1}), public_key(std::in_place_index<1>));
   test_key(public_key(sysio::webauthn_public_key{{}, sysio::webauthn_public_key::user_presence_t::USER_PRESENCE_NONE, "b"}),
            public_key(sysio::webauthn_public_key{{}, sysio::webauthn_public_key::user_presence_t::USER_PRESENCE_PRESENT, "a"}));
