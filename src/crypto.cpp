...
#include <array>
#include <cstdint>
#include "../include/sysio/crypto.hpp"
#include "../include/sysio/from_bin.hpp"
#include "../include/sysio/from_json.hpp"
#include "../include/sysio/to_bin.hpp"
#include "../include/sysio/to_json.hpp"
#include <string>
#include <string_view>
...
void base58_to_binary(Container& result, std::string_view s) {
    ...
    for (auto& src_digit : s) {
        ...
        check(carry >= 0,
            ::sysio::convert_json_error(::sysio::from_json_error::expected_key));
...
std::array<unsigned char, 20> digest_suffix_ripemd160(const Container&... data) {
    ...
    check( abieos_ripemd160::ripemd160_digest(&self, digest.data()),
        convert_json_error(sysio::from_json_error::invalid_signature) );
    return digest;
}
...
Key string_to_key(std::string_view s, key_type type, std::string_view suffix) {
    ...
    check(whole.size() > 5,
        convert_json_error(sysio::from_json_error::expected_key));
...
std::string sysio::public_key_to_string(const public_key& key) {
...
std::string sysio::private_key_to_string(const private_key& private_key) {
...
private_key sysio::private_key_from_string(std::string_view s) {
...
std::string sysio::signature_to_string(const sysio::signature& signature) {
...
signature sysio::signature_from_string(std::string_view s) {
