...
inline uint64_t string_to_symbol_code(const char* s) {
    uint64_t result;
    if (!sysio::string_to_symbol_code(result, s, s + strlen(s)))
...
inline uint64_t string_to_symbol(const char* s) {
    uint64_t result;
    if (!sysio::string_to_symbol(result, s, s + strlen(s)))
...
inline asset string_to_asset(const char* s) {
    asset result;
    if (!sysio::string_to_asset(result.amount, result.sym.value, s, s + strlen(s)))
